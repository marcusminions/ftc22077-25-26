...
public class Bpad {
    ...
    public void update(Gamepad gpad) {
        if(this.gpad == null) this.gpad=gpad;
        ...
        map.put("db_left_trigger", gpad.left_trigger!=0 && !map.get("last_left_trigger"));

        map.put("last_right_trigger", map.get("right_trigger"));
        map.put("right_trigger", gpad.right_trigger != 0);
        map.put("db_right_trigger", gpad.right_trigger!=0 && !map.get("last_right_trigger"));
        ...
        map.put("db_left_stick_x", gpad.left_stick_x!=0 && map.get("last_left_stick_x"));

        map.put("last_left_stick_y", map.get("left_stick_y"));
        map.put("left_stick_y", gpad.left_stick_y != 0);
        map.put("db_left_stick_y", gpad.left_stick_y!=0 && map.get("last_left_stick_y"));

        map.put("last_right_stick_x", map.get("right_stick_x"));
        map.put("right_stick_x", gpad.right_stick_x != 0);
        //map.put("db_right_stick_x", gpad.right_stick_x!=0 && map.get("last_right_stick_x"));

        map.put("last_right_stick_y", map.get("right_stick_y"));
        map.put("right_stick_y", gpad.right_stick_y != 0);
        map.put("db_right_stick_x", gpad.right_stick_y!=0 && map.get("last_right_stick_y"));
    }
}